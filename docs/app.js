...
const API_BASE_URL = queryParams.get('api') || (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:8000' : null);
const HAS_API = API_BASE_URL !== null;

if (!HAS_API) {
    console.warn('⚠️ GitHub Pages exige um backend API externo. Use ?api=https://seu-backend.com para conectar.');
}
...
async function preencherFigurinhas() {
    try {
        if (!HAS_API) {
            throw new Error('API não configurada. Use ?api=https://seu-backend.com na URL.');
        }
