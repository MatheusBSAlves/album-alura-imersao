...
async function preencherFigurinhas() {
  try {
    if (!HAS_API) {
      throw new Error(
        "API não configurada. Use ?api=https://seu-backend.com na URL.",
      );
    }
    ...
    console.warn(
      "⚠️  Não foi possível conectar à API do backend:",
      erro.message,
    );
    console.info(
      "ℹ️  Inicie o servidor: cd backend/dia-3 && uvicorn main:app --reload",
    );
  }
}
  ...
  function playPaperTurnSound() {
    ...
    try {
      ...
      for (let i = 0; i < bufferSize; i++) {
        ...
        const paperCrackle =
          Math.random() > 0.985 ? (Math.random() * 2 - 1) * 0.35 : 0;
      ...
      bandpassFilter.frequency.exponentialRampToValueAtTime(
        350,
